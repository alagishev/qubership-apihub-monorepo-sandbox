...
type ActivityTrackingService interface {
	...
	GetActivityHistory_deprecated_2(ctx context.SecurityContext, req view.ActivityHistoryReq) (*view.PkgActivityResponse, error)
	...
	GetEventsForPackage_deprecated_2(packageId string, includeRefs bool, limit int, page int, textFilter string, types []string) (*view.PkgActivityResponse, error)
...
func (a activityTrackingServiceImpl) GetActivityHistory_deprecated_2(ctx context.SecurityContext, req view.ActivityHistoryReq) (*view.PkgActivityResponse, error) {
	var ids []string
	...
	return a.makePkgActivityResponse_deprecated_2(ents)
}
...
func (a activityTrackingServiceImpl) GetEventsForPackage_deprecated_2(packageId string, includeRefs bool, limit int, page int, textFilter string, typeGroups []string) (*view.PkgActivityResponse, error) {
	pkgEnt, err := a.publishedRepo.GetPackage(packageId)
	if err != nil {
		return nil, fmt.Errorf("failed to get package %s for events: %w", packageId, err)
	}
	...
	atTypes := view.ConvertEventTypes_deprecated(typeGroups)
...
func (a activityTrackingServiceImpl) makePkgActivityResponse_deprecated_2(ents []entity.EnrichedActivityTrackingEntity) (*view.PkgActivityResponse, error) {
	result := view.PkgActivityResponse{}
	result.Events = make([]view.PkgActivityResponseItem, len(ents))
	for i, ent := range ents {
		if ent.Type == string(view.ATETDeleteRevision) {
			continue
		}
		if ent.Type == string(view.ATETPublishNewRevision) ||
			ent.Type == string(view.ATETPublishNewVersion) ||
		...
		result.Events[i] = entity.MakeActivityTrackingEventView_deprecated_2(ent)
	}
	return &result, nil
}
