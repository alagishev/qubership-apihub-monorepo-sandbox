...
2. **Check Postgres** is reachable at `localhost:5432` (matches
   `qubership-apihub-backend/qubership-apihub-service/local_config/config.yaml`). If unreachable, **stop and ask the
...
3. **Build**: `go build .` from `qubership-apihub-backend/qubership-apihub-service/` (do not force
...
4. **Start locally with `local_config`**: set `APIHUB_CONFIG_FOLDER` to
   `qubership-apihub-backend/qubership-apihub-service/local_config` and run the built binary from
   `qubership-apihub-backend/qubership-apihub-service/`. Default listen address is `:8090`. There is no env-var override
