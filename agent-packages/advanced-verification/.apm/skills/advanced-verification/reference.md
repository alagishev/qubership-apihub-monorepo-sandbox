...
`qubership-apihub-backend/qubership-apihub-service/local_config/config.yaml`.
...
Start with the local config, from `qubership-apihub-backend/qubership-apihub-service/`:
...
```bash
cp -r qubership-apihub-backend/qubership-apihub-service/local_config /tmp/local_config_override
